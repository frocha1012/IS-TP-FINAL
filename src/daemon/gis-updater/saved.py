...
def fetch_coordinates_for_country(country_name):
    ...
    if response.status_code == 200 and response.json():
        ...
        print(f"Fetched coordinates for {country_name}: Latitude {lat}, Longitude {lon}")
        return lat, lon
    else:
        print(f"Error fetching coordinates for {country_name}: {response.status_code}, {response.text}")
        return None, None
...
def update_country_coordinates(country_id, country_name, lat, lon):
    ...
    if response.status_code in [200, 201]:
        ...
        print(f"Coordinates updated for {country_name} (ID: {country_id}). Latitude: {lat}, Longitude: {lon}")
    else:
        print(f"Failed to update coordinates for {country_name} (ID: {country_id}): {response.status_code}, {response.text}")
...
if __name__ == "__main__":
    while True:
        ...
        if countries:
            print(f"Found {len(countries)} countries without coordinates.")
            for country in countries:
                ...
                if lat and lon:
                    ...
                    print(f"Could not retrieve coordinates for {country['name']}")
        else:
            print("No countries found that need coordinates updated.")
